...
use App\Domains\EmiRequest\Pdf\Registry\BankPdfGeneratorRegistry;
use App\Domains\EmiRequest\Pdf\Renderer\HtmlPassthroughPdfRenderer;
use App\Domains\EmiRequest\ReadModels\EmiRequestReadModel;
use App\Domains\EmiRequest\Schemas\BankSchemaRegistry;
use App\Domains\EmiRequest\Schemas\Global\GlobalSchema;
use App\Domains\EmiRequest\Schemas\Nabil\NabilSchema;
use App\Domains\EmiRequest\Schemas\Sbl\SblSchema;

final class GenerateEmiApplicationPdfAction
{
    public function __construct(
        private readonly EmiRequestReadModel $emiRequestReadModel,
    ) {}

    public function execute(string $emiRequestId): PdfResultDto
    {
        $emiRequest = $this->emiRequestReadModel->getForApplication($emiRequestId);
        ...
        $schemaRegistry = new BankSchemaRegistry([
            new NabilSchema(),
            new SblSchema(),
            new GlobalSchema(),
        ]);

        $renderer = new HtmlPassthroughPdfRenderer();
        $pdfRegistry = new BankPdfGeneratorRegistry([
            new NabilPdfGenerator($renderer),
            new SiddarthaPdfGenerator($renderer),
            new GlobalPdfGenerator($renderer),
        ]);

        $bankData = $schemaRegistry->get($bankCode)->toData($payload);
        $pdf = $pdfRegistry->for($bankCode)->generate($bankData, $emiRequest);
