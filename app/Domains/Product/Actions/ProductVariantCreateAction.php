...
final class ProductVariantCreateAction
{
    /**
     * @param array<string, mixed> $attributes
     * @return array{message: string, data: array<string, mixed>, success: bool}
     */
    public function execute(int $productId, float $price, int $quantity, array $attributes = []): array
    {
        Product::query()->findOrFail($productId);
        $normalizedAttributes = $this->normalizeAttributes($attributes);

        if ($this->hasDuplicateAttributes($productId, $normalizedAttributes)) {
     ...
     */
    private function hasDuplicateAttributes(int $productId, array $normalizedAttributes): bool
    {
