...
final class CreateProductVariantHandler
{
     ...
     */
    public function handle(CreateProductVariantCommand $cmd): array
    {
        ProductModel::query()->findOrFail($cmd->productId);
        $normalizedAttributes = $this->normalizeAttributes($cmd->attributes);

        if ($this->hasDuplicateAttributes($cmd->productId, $normalizedAttributes)) {
