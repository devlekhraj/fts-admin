<template>
  <AppPageHeader title="Cart Detail" :subtitle="`Cart #${id}`">
    <template #actions>
      <v-btn variant="flat" color="primary" @click="$router.push({ name: 'admin.orders.cart' })">
          ...
          <div class="d-flex align-center justify-space-between flex-wrap ga-3 mb-4">
            <div class="d-flex align-center ga-3">
              <v-avatar size="40" color="grey-lighten-3" rounded>
                <v-img v-if="cart?.customer?.avatar" :src="cart.customer.avatar"
                  :alt="cart?.customer?.name ?? 'Customer'" cover />
                  ...
                  <td>
                    <div class="d-flex align-center ga-3 py-2">
                      ...
                      <div class="d-flex flex-column">
                        ...
                        <div v-if="item.product_attributes && Object.keys(item.product_attributes).length"
                          class="text-caption text-medium-emphasis">
                          <span v-for="(attrEntry, idx) in Object.entries(item.product_attributes)"
                            :key="`${item.id}-${attrEntry[0]}`">
                            <strong>{{ attrEntry[0] }}:</strong> {{ attrEntry[1] }}<span
                              v-if="idx < Object.entries(item.product_attributes).length - 1"> • </span>
                          </span>
                        </div>
                      </div>
                    </div>
