...
import { deleteBrandImage, updateBrandImage } from '@/api/products.api';
...
async function onUpdate() {
  ...
  updating.value = true;
  try {
    await updateBrandImage(brandId, fileUsageId, {
      alt_text: String(form.alt_text ?? '').trim(),
      is_default: Boolean(form.is_default),
    });

    snackbar.show({ message: 'Brand image updated successfully.', color: 'success' });
...
async function onDelete() {
  ...
  deleting.value = true;
  try {
    await deleteBrandImage(brandId, fileUsageId);

    snackbar.show({ message: 'Brand image deleted successfully.', color: 'success' });
