    ...
    <div class="mt-3 d-flex align-center justify-space-around py-6">
      <div style="width: max-content;" class="d-flex align-center">
        <v-avatar size="40" color="grey-lighten-3" class="mr-3">
          <v-icon size="22" color="grey-darken-1">mdi-tag-multiple-outline</v-icon>
        </v-avatar>
        <div>
          <div class="font-weight-medium text-capitalize">{{ brand.name ?? '-' }}</div>
          <div class="text-medium-emphasis">{{ brand.slug ?? '' }}</div>
        </div>
      </div>
    </div>
    <div class="text-body-2 text-medium-emphasis text-center">
      This action will soft delete the brand. Continue?
    </div>
  </v-card-text>
...
import { ref } from 'vue';
import { deleteBrand } from '@/api/products.api';
...
async function onConfirm() {
  ...
  loading.value = true;
  try {
    const response = await deleteBrand(String(props.brand.id));
    const message = (response as any)?.data?.message ?? 'Brand deleted successfully.';
    snackbar.show({ message, color: 'success' });
    emit('saved', { id: props.brand.id });
    emit('close');
  } catch (err) {
    ...
    if (response?.status === 422 && responseErrors && typeof responseErrors === 'object') {
      ...
      const message = next.id?.[0] ?? 'Failed to delete brand.';
      error.value = message;
