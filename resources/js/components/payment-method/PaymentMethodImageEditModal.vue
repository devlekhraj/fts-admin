...
import { deletePaymentMethodImage, updatePaymentMethodImage } from '@/api/payment-methods.api';
import AppFieldLabel from '@/components/shared/AppFieldLabel.vue';
import { getErrorMessage } from '@/shared/errors';
import { useSnackbarStore } from '@/stores/snackbar.store';

type PaymentMethodFile = {
  id: number | string;
  url?: string | null;
...
async function onUpdate() {
  ...
  if (!paymentMethodId) {
    snackbar.show({ message: 'Payment method id is required.', color: 'error' });
    return;
  }
  ...
  updating.value = true;
  try {
    await updatePaymentMethodImage(paymentMethodId, fileUsageId, {
      alt_text: String(form.alt_text ?? '').trim(),
      is_default: Boolean(form.is_default),
    });

    snackbar.show({ message: 'Payment method image updated successfully.', color: 'success' });
...
async function onDelete() {
  ...
  deleting.value = true;
  try {
    await deletePaymentMethodImage(paymentMethodId, fileUsageId);

    snackbar.show({ message: 'Payment method image deleted successfully.', color: 'success' });
