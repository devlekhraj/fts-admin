<template>
  <div class="pa-6">
    ...
    <v-table v-if="blogFiles.length" density="comfortable">
      ...
      <tbody>
        <tr v-for="file in blogFiles" :key="String(file.id)">
          <td class="py-3">
    ...
    <div v-else class="empty-images-state">
      ...
      <div class="text-body-2 text-medium-emphasis">No files attached to this blog.</div>
    </div>
  </div>
</template>
...
function onEditFile(file: NonNullable<BlogDetailResponse['files']>[number]) {
  openModal(
    BlogImageEditModal,
    {
      blogId: props.item?.id ?? null,
      file,
    },
    {
