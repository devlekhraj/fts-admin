...
import { deleteBlogImage, updateBlogImage } from '@/api/blogs.api';
...
async function onUpdate() {
  ...
  if (blogId === null || blogId === undefined || String(blogId).trim() === '') {
    snackbar.show({ message: 'Blog id is required.', color: 'error' });
    return;
  }
  ...
  updating.value = true;
  try {
    await updateBlogImage(blogId, fileUsageId, {
      alt_text: String(form.alt_text ?? '').trim(),
      is_default: Boolean(form.is_default),
    });

    snackbar.show({ message: 'Blog image updated successfully.', color: 'success' });
...
async function onDelete() {
  ...
  deleting.value = true;
  try {
    await deleteBlogImage(blogId, fileUsageId);

    snackbar.show({ message: 'Blog image deleted successfully.', color: 'success' });
