<template>
  <v-card-text class="py-6">
    ...
    <div class="mt-3 d-flex align-center justify-space-around py-6">
      <div style="width: max-content;" class="d-flex align-center">
        <v-avatar size="40" color="grey-lighten-3" class="mr-3">
          <v-icon size="22" color="grey-darken-1">mdi-folder-outline</v-icon>
        </v-avatar>
        <div>
    ...
    <div class="text-body-2 text-medium-emphasis text-center">
      This action cannot be undone. Do you want to delete this blog category?
    </div>
  </v-card-text>
...
import { ref } from 'vue';
import { remove as deleteBlogCategory } from '@/api/blog-categories.api';
...
async function onConfirm() {
  ...
  loading.value = true;
  try {
    const response = await deleteBlogCategory(String(props.category.id));
    const message = (response as any)?.data?.message ?? 'Blog category deleted successfully.';
    ...
    if (response?.status === 422 && responseErrors && typeof responseErrors === 'object') {
      ...
      const message = next.id?.[0] ?? 'Failed to delete blog category.';
      error.value = message;
