<template>
  <BaseDetailTabImages :files="categoryFiles" usage-type="blog_categories" :usage-id="item?.id ?? null"
    :edit-modal="BlogCategoryImageEditModal" :edit-modal-props="(file) => ({ categoryId: item?.id ?? null, file })"
    edit-modal-title="Edit Blog Category Image" empty-state-message="No files attached to this category." show-file-id
    ...
    <template #rows="{ file }">
      <td class="py-3">
        <v-chip size="small" label variant="tonal" :color="file.meta?.is_active ? 'success' : 'warning'">
