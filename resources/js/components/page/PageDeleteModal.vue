  ...
  <v-card-text class="py-6">
    ...
    <div class="mt-3 d-flex align-center justify-space-around py-6">
      <div style="width: max-content;" class="d-flex align-center">
        ...
        <div>
          <div class="font-weight-medium text-capitalize">{{ page.title ?? '-' }}</div>
          <div class="text-medium-emphasis">{{ page.slug ?? '' }}</div>
        </div>
      </div>
    </div>
    <div class="text-body-2 text-medium-emphasis text-center">
      This action will delete the page. Continue?
    </div>
  </v-card-text>
...
async function onConfirm() {
  error.value = '';
  if (!props.page?.id) {
    error.value = 'Page id is required.';
    return;
  }

  loading.value = true;
  try {
    const response = await deletePage(String(props.page.id));
    const message = (response as any)?.data?.message ?? 'Page deleted successfully.';
    snackbar.show({ message, color: 'success' });
    emit('saved', { id: props.page.id });
    emit('close');
  } catch (err) {
    ...
    if (response?.status === 422 && responseErrors && typeof responseErrors === 'object') {
      ...
      const message = next.id?.[0] ?? 'Failed to delete page.';
      error.value = message;
