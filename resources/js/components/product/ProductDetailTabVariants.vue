          ...
          <tr>
            ...
            <td class="py-3">
              <v-chip
                ...
                <v-icon end size="14">{{ isExpanded(variant) ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
              </v-chip>
            </td>
            <td class="py-3">
              <div class="d-flex align-center ga-2">
                <v-btn icon size="x-small" variant="tonal" color="primary" @click="onViewVariant(variant)">
                  <v-icon size="16">mdi-eye</v-icon>
                </v-btn>
                <v-btn icon size="x-small" variant="tonal" color="error" @click="onDeleteVariant(variant)">
              ...
              <div class="container">
                <v-row>
                  <v-col cols="12" lg="8" offset-lg="2">
                    <div class="py-14">
                      <v-table v-if="getVariantImageCount(variant)" density="compact" class="variant-images-table border rounded">
                        <thead>
                          <tr>
                            <th>Image</th>
                            <th>Description</th>
                        ...
                        <tbody>
                          <tr v-for="image in variant.images" :key="String(image.id)">
                            <td class="py-2 image-col">
                            ...
                            <td class="py-2">
                              ...
                              <div class="text-caption text-medium-emphasis mt-2">
                                {{ formatBytes(image.file_size ?? image.size) }} | {{ Number(image.width ?? 0) }} x
                                {{ Number(image.height ?? 0) }}
                              </div>
                            </td>
                            <td class="py-2 action-col">
                              <v-btn
                                v-if="image.url"
