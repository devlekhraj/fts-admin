...
import { deleteProductImage, updateProductImage } from '@/api/products.api';
...
async function onUpdate() {
  ...
  if (!productId) {
    snackbar.show({ message: 'Product id is required.', color: 'error' });
    return;
  }
  ...
  updating.value = true;
  try {
    await updateProductImage(productId, fileUsageId, {
      alt_text: String(form.alt_text ?? '').trim(),
      is_default: Boolean(form.is_default),
    });

    snackbar.show({ message: 'Product image updated successfully.', color: 'success' });
...
async function onDelete() {
  ...
  deleting.value = true;
  try {
    await deleteProductImage(productId, fileUsageId);

    snackbar.show({ message: 'Product image deleted successfully.', color: 'success' });
