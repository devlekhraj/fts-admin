...
import { deleteVariantImage, updateVariantImage } from '@/api/products.api';
...
async function onUpdate() {
  ...
  if (!variantId) {
    snackbar.show({ message: 'Variant id is required.', color: 'error' });
    return;
  }
  ...
  updating.value = true;
  try {
    await updateVariantImage(variantId, fileUsageId, {
...
async function onDelete() {
  ...
  deleting.value = true;
  try {
    await deleteVariantImage(variantId, fileUsageId);
